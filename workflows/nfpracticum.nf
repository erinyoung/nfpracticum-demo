...
include { SEROBA_RUN              } from '../modules/nf-core/seroba/run/main'
...
workflow NFPRACTICUM {
    ...
    //
    // MODULE: SEROBA (SEROBA_RUN)
    //

    SEROBA_RUN(ch_filtered_reads)
    ...
    SEROBA_RUN.out.tsv
        .map {it -> it[1]}
        .collectFile(
            storeDir: "${params.outdir}/seroba",
            name: "seroba_summary.tsv",
            sort: true,
            keepHeader: false
        )
